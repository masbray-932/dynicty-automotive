...
export default async function CarDetailPage({ params }: PageProps) {
  ...
  const publicUrl = `${(process.env.NEXT_PUBLIC_SITE_URL ?? "http://localhost:3000").replace(/\/$/, "")}/cars/${car.slug}`;
  ...
  const structuredData = {
    "@context": "https://schema.org", "@type": "Vehicle", name: car.title,
    ...
    brand: { "@type": "Brand", name: car.brand.name }, model: car.model.name, vehicleModelDate: String(car.year),
    vehicleTransmission: car.transmission, fuelType: car.fuelType,
    ...(shouldShowMileage(car.condition, car.mileage) ? { mileageFromOdometer: { "@type": "QuantitativeValue", value: car.mileage, unitCode: "KMT" } } : {}),
    offers: { "@type": "Offer", price: car.price, priceCurrency: "IDR", availability: "https://schema.org/InStock", url: publicUrl },
  };
  ...
  return (
    <PageContainer className={`py-8 sm:py-12 ${whatsappUrl ? "pb-28 lg:pb-16" : ""}`}>
      ...
      {whatsappUrl ? <div className="fixed inset-x-0 bottom-0 z-40 border-t border-white/10 bg-zinc-950/95 p-3 backdrop-blur lg:hidden"><a className="mx-auto flex min-h-12 max-w-md items-center justify-center rounded-xl bg-emerald-600 px-5 font-bold text-white focus-visible:outline-2 focus-visible:outline-white" href={whatsappUrl} rel="noreferrer" target="_blank">Chat WhatsApp</a></div> : null}
    </PageContainer>
  );
}
