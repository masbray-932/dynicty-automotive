...
export async function saveDealerSettingsAction(_state: ActionState, formData: FormData): Promise<ActionState> {
  ...
  catch { logger.error("dealer.settings_save_failed"); return { message: "Pengaturan dealer tidak dapat disimpan.", errors: {} }; }
}
...
export async function uploadDealerLogoAction(_state: ActionState, formData: FormData): Promise<ActionState> {
  ...
  if (!parsed.success) { logger.warn("dealer.logo_rejected"); return { message: "Logo tidak dapat diunggah.", errors: { logo: [parsed.message] } }; }
  try { await replaceDealerLogo(parsed.data); refreshSettingsConsumers(); return { message: "Logo dealer berhasil diperbarui.", errors: {} }; }
  catch { logger.error("dealer.logo_upload_failed"); return { message: "Logo dealer tidak dapat disimpan.", errors: {} }; }
}
...
export async function removeDealerLogoAction(state: ActionState, formData: FormData): Promise<ActionState> {
  ...
  void formData;
  await requireAdmin();
  try { const result = await removeDealerLogo(); refreshSettingsConsumers(); if (result.cleanupFailed) logger.warn("dealer.logo_cleanup_failed"); return { message: result.cleanupFailed ? "Logo dihapus, tetapi file lama perlu dibersihkan manual." : "Logo dealer berhasil dihapus.", errors: {} }; }
  catch { logger.error("dealer.logo_remove_failed"); return { message: "Logo dealer tidak dapat dihapus.", errors: {} }; }
}
