...
Phase 6 also applies traversal validation to R2 public URL generation. External media remains unoptimized until the exact production hostname is known; Next Image therefore has no unsafe wildcard host. Local storage is rejected by production environment validation. The R2 adapter's network operations remain a mandatory Phase 7 item.
